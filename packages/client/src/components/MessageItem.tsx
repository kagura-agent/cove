...
export function MessageItem({ message }: { message: Message }) {
  ...
  return (
    <div className={`max-w-[75%] max-sm:max-w-[80%] px-3.5 py-2.5 rounded-[14px] text-sm leading-relaxed break-words animate-fade-in ${isSelf ? "bg-msg-own self-end rounded-br-[4px]" : "bg-msg-other self-start rounded-bl-[4px]"}`}>
      <div className={`text-xs font-semibold mb-0.5 ${isSelf ? "text-purple-300" : "text-accent"}`}>{message.author.username}</div>
      <div className="whitespace-pre-wrap">{message.content}</div>
      <div className="text-[10px] text-text-dim mt-1 text-right">{formatTime(message.timestamp)}</div>
    </div>
  );
}
