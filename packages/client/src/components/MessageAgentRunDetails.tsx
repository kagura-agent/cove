...
function ExecutionDetailsSurface({ narrow, placement, surfaceRef, titleId, onClose, events }: { narrow: boolean; placement: ExecutionDetailsPlacement | null; surfaceRef: React.RefObject<HTMLElement | null>; titleId: string; onClose: () => void; events: NonNullable<AgentRunTimeline["events"]> }) {
  ...
  const heading = <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "var(--space-sm)", paddingBottom: "var(--space-xs)", borderBottom: "1px solid var(--border-subtle)" }}><strong id={titleId} style={{ fontSize: "var(--font-size-sm)" }}>Execution details</strong>{closeButton}</header>;
  const timeline = <div style={{ overflow: "auto", maxHeight: narrow ? "min(58vh, 460px)" : placement?.maxHeight ?? 360, paddingTop: "var(--space-xs)" }}><ExecutionTimeline events={events} /></div>;
  if (narrow) return <div style={{ position: "fixed", zIndex: 10000, inset: 0, display: "flex", alignItems: "flex-end", background: "rgba(0,0,0,.42)" }}>
    <section ref={surfaceRef as React.RefObject<HTMLElement>} role="dialog" aria-modal="true" aria-labelledby={titleId} style={{ width: "100%", maxHeight: "82vh", overflow: "hidden", background: "var(--bg-floating)", borderRadius: "16px 16px 0 0", boxShadow: "0 -8px 28px rgba(0,0,0,.35)", padding: "var(--space-md)" }}>{heading}{timeline}</section>
  </div>;
  return <section ref={surfaceRef as React.RefObject<HTMLElement>} role="dialog" aria-labelledby={titleId} style={{ position: "fixed", zIndex: 10000, top: placement?.top ?? VIEWPORT_GUTTER, left: placement?.left ?? VIEWPORT_GUTTER, width: placement?.width ?? POPOVER_WIDTH, maxWidth: `calc(100vw - ${VIEWPORT_GUTTER * 2}px)`, background: "var(--bg-floating)", border: "1px solid var(--border-subtle)", borderRadius: "var(--space-sm)", boxShadow: "0 8px 24px rgba(0,0,0,.35)", padding: "var(--space-sm)" }}>{heading}{timeline}</section>;
}
