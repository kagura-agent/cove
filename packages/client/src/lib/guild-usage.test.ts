...
const day = (date: string, cost: number | null, tokens: number, calls: number, models: Array<{ model: string; cost: number | null }>): GuildDailyUsage => ({
  date, cost, total_tokens: tokens, calls,
...
describe("flattenDailyForChart", () => {
  it("maps date to MM-DD and keeps full date", () => {
    const rows = flattenDailyForChart([day("2026-08-21", 0.01, 100, 1, [])]);
    expect(rows[0]).toMatchObject({ date: "08-21", fullDate: "2026-08-21", cost: 0.01, tokens: 100, calls: 1 });
  });
