...
import type { AgentRunTimeline, AgentRunUsage, CreateTaskFields, GuildDailyUsage, GuildUsageOverview, RecurringTask, RecurringTaskOccurrenceMode, Role, Task, TaskEfficiencyReport, TaskRunStat, TaskStatus, UpdateTaskFields, Webhook } from "@cove/shared";
...
export function fetchGuildUsageOverview(guildId: string) {
  return api<GuildUsageOverview>(`${API_PREFIX}/guilds/${guildId}/usage/overview`);
}
export function fetchGuildUsageDaily(guildId: string, days = 14) {
  return api<GuildDailyUsage[]>(`${API_PREFIX}/guilds/${guildId}/usage/daily?days=${days}`);
}
