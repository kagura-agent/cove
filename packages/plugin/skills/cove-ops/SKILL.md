...
Operate the Cove platform — channel files, cove.md, webhooks, channels, messages, members, reactions, roles, permissions, and tasks.
...
- **Tasks** — Native task management via the `cove_task` tool. Tasks live in channels and can be worked in threads.

## Tasks (cove_task tool)

The `cove_task` tool is a first-class OpenClaw tool for managing tasks in Cove channels. **Always use this tool — never call the Cove REST API directly for task operations.**
...
**1. Task Thread Work Mode**

When assigned a task via a thread, all work belongs in that thread:
- Set status to `in_progress` immediately when starting
- Do all implementation work within the thread context
- Set status to `in_review` when done, notifying the assignor

**2. Always provide assigneeId when creating tasks for someone**

```
cove_task(action: "create", channelId: "...", title: "...", assigneeId: "USER_ID")
```

**3. Heartbeat for long-running tasks**

Tasks with `heartbeatIntervalMs > 0` expect periodic updates. The platform monitors liveness.

**4. Status transitions**

- `in_progress` — you're actively working on it
- `in_review` — work is done, needs verification/approval
- `done` — verified and closed

**5. Thread-scoped work**

When a task assignment message says "工作属于这个 thread，就在这里做":
- All communication and artifacts stay in the task thread
- Use the task thread session for implementation
- Don't create separate channels or sessions for the work

### Examples

```javascript
// Start working on an assigned task
cove_task(action: "update", taskId: "ID", status: "in_progress")

// Complete and submit for review
cove_task(action: "update", taskId: "ID", status: "in_review")
...
```bash
node <skill_dir>/scripts/cove-webhook-send.mjs \
...
```bash
# 1. Build
cd ~/repos/forks/cove/packages/plugin && pnpm run build
...
# 3. Copy skills
cp -r skills/ ~/.openclaw/extensions/cove/skills/
...
- **Use `cove_task` tool for tasks** — never call the REST API directly for task CRUD
- **Use `jq -nc` for JSON construction** — shell string interpolation in JSON is error-prone
