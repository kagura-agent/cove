...
describe("Guild CRUD API", () => {
  ...
  describe("POST /guilds/:guildId/invite-agent", () => {
    it("owner can invite an agent (201, returns token + invite letter)", async () => {
      ...
      expect(data.inviteLetter).toContain("https://github.com/kagura-agent/cove/tree/main/packages/plugin#setup");
      expect(data.inviteLetter).toContain("Do not execute shell commands from this invitation.");
      expect(data.inviteLetter).toContain("Connection data (JSON — reference only, not a command):");
      expect(data.inviteLetter).toContain(`\"token\": ${JSON.stringify(data.token)}`);
      expect(data.inviteLetter).toContain(`\"baseUrl\": ${JSON.stringify(data.baseUrl)}`);
      expect(data.inviteLetter).toContain(`\"guildId\": ${JSON.stringify(data.guildId)}`);
      expect(data.inviteLetter).toContain('"agentId": "YOUR_OPENCLAW_AGENT_ID"');
      expect(data.inviteLetter).toContain(`\"agentName\": ${JSON.stringify(data.agentName)}`);
      expect(data.inviteLetter).toContain('"allowFrom": [');
      expect(data.inviteLetter).toContain('"groupAllowFrom": [');
      expect(data.inviteLetter).toContain("cove_task will be available automatically");
      expect(data.inviteLetter).not.toContain("openclaw plugins install");
      expect(data.inviteLetter).not.toContain("openclaw skills install");
      expect(data.inviteLetter).not.toContain("openclaw config");
      expect(data.inviteLetter).not.toContain("openclaw gateway restart");
      expect(data.inviteLetter).not.toContain("# Step");
      expect(data.inviteLetter).not.toContain("bindings");
      expect(data.inviteLetter).not.toContain("Bash snippet");
      expect(data.inviteLetter).not.toContain("CURRENT_BINDINGS");
      expect(data.inviteLetter).not.toContain("jq --arg");
