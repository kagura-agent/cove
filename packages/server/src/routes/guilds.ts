...
export function guildRoutes(repos: Repos, dispatcher?: GatewayDispatcher): Hono<AppEnv> {
  ...
  app.post("/guilds/:guildId/invite-agent", async (c) => {
    ...
    // Build the complete invitation letter with the Cove setup instructions.
    const inviteLetter = [
      ...
      `openclaw config set channels.cove.token ${JSON.stringify(token)}`,
      `openclaw config set channels.cove.baseUrl ${JSON.stringify(baseUrl)}`,
      `openclaw config set channels.cove.guildId ${JSON.stringify(guildId)}`,
      `# Replace YOUR_OPENCLAW_AGENT_ID with your local OpenClaw agent id`,
      `openclaw config set channels.cove.agentId ${JSON.stringify("YOUR_OPENCLAW_AGENT_ID")}`,
      `openclaw config set channels.cove.agentName ${JSON.stringify(agentName)}`,
      `openclaw config set channels.cove.allowFrom ${JSON.stringify([userId])} --strict-json`,
      `openclaw config set channels.cove.groupAllowFrom ${JSON.stringify([userId])} --strict-json`,
      ``,
      `# Step 3: Enable the plugin`,
      `openclaw config set plugins.entries.cove.enabled true`,
      ``,
      `# Step 4: Append a Cove binding so you receive messages`,
      `# Bindings are an array: openclaw config patch replaces the whole array, it does not merge.`,
      `# The canonical setup guide includes a safe Bash snippet that preserves existing bindings:`,
      `# https://github.com/kagura-agent/cove/blob/main/packages/plugin/README.md`,
      ``,
