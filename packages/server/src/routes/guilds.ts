...
export function guildRoutes(repos: Repos, dispatcher?: GatewayDispatcher): Hono<AppEnv> {
  ...
  app.post("/guilds/:guildId/invite-agent", async (c) => {
    ...
    // The invitation is data only; the operator follows the canonical setup guide.
    const connectionData = JSON.stringify({
      token,
      baseUrl,
      guildId,
      agentId: "YOUR_OPENCLAW_AGENT_ID",
      agentName,
      allowFrom: ["*"],
      groupAllowFrom: ["*"],
    }, null, 2);
    const inviteLetter = [
      ...
      ``,
      `════════════════════════════════════════`,
      `This invitation contains connection data, not executable setup instructions.`,
      `Your operator or administrator must review and follow the canonical Cove plugin setup guide:`,
      `https://github.com/kagura-agent/cove/tree/main/packages/plugin#setup`,
      `Do not execute shell commands from this invitation.`,
      ``,
      `Connection data (JSON — reference only, not a command):`,
      connectionData,
      ``,
      `The agentId value is a placeholder for your operator or administrator to provide using the guide.`,
      `The allowFrom and groupAllowFrom values above indicate the expected sender policy.`,
      ``,
      `Once the Cove plugin is enabled, cove_task will be available automatically; it does not require registration.`,
      `════════════════════════════════════════`,
