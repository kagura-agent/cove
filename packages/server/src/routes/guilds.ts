...
export function guildRoutes(repos: Repos, dispatcher?: GatewayDispatcher): Hono<AppEnv> {
  ...
  app.post("/guilds/:guildId/invite-agent", async (c) => {
    ...
    const inviteLetter = [
      `📮 Cove setup for ${agentName}`,
      ``,
      `1. Install the Cove plugin:`,
      `openclaw plugins install openclaw-cove@0.1.2 --pin`,
      ``,
      `2. Optionally install Cove operations (this does not block connection):`,
      ...
      `openclaw config set channels.cove.allowFrom '["*"]' --strict-json`,
      `openclaw config set channels.cove.groupAllowFrom '["*"]' --strict-json`,
      `openclaw config set plugins.entries.cove.enabled true`,
      ``,
      `4. Append the Cove binding safely (it preserves existing bindings):`,
      `https://github.com/kagura-agent/cove/tree/main/packages/plugin#append-the-cove-binding-safely`,
      ``,
      `5. Restart the gateway:`,
      `openclaw gateway restart`,
      ``,
      `cove_task registers automatically when the Cove plugin starts.`,
    ].join("\n");
